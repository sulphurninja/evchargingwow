...
export default function Login() {
    const initialState = { vendorEmail: "", password: "", latitude: "", longitude: "" };
    const [vendorData, setVendorData] = useState(initialState);
    const [error, setError] = useState(false);
    const { vendorEmail, password, latitude, longitude } = vendorData;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Make login request
            const res = await postData('auth/loginVendor', vendorData);
            ...
            if (res.vendor) {
                router.push("/vendor/dashboard"); // Redirect to admin page
                toast("🙏 Welcome Vendor!");
            } else {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                position => {
                    ...
                    setVendorData({ ...vendorData, latitude, longitude });
                },
                error => {
        ...
        <div className="relative h-screen flex-col items-center justify-center md:grid lg:max-w-none lg:grid-cols-2 lg:px-0">
            ...
            <div className="relative hidden h-full flex-col bg-muted p-10 text-white dark:border-r lg:flex">
              ...
              before.&rdquo;
                        ...
                        <footer className="text-sm">Vendor Dashboard</footer>
                    </blockquote>
                </div>
            </div>
                ...
                <div className="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[350px]">
                    ...
                    <form onSubmit={handleSubmit} className="bg-[#18181B] rounded-xl p-6  space-y-4">
                        ...
                        <div className="flex flex-col text-white">
                            <label className="">Vendor Email</label>
                        ...
                        <button className='bg-white/90 2  mt-2 rounded-md text-black p-2 w-full' type="button" onClick={getLocation}>Get Location</button>
                        <div className='flex justify-center '>
                            <button type="submit" className="bg-white/90 2  mt-2 rounded-md text-black p-2 w-full">Submit</button>
                        </div>
                    </form>
                    {error && <p className='text-red-500 font-bold flex gap-2'><MapIcon /> Please provide your location!</p>}
