...
export default function TotalStock() {
    ...
    return (
        <LayoutVendor>
            <div className='p-4'>
                <h1 className='text-center font-bold'>Total Stock</h1>
                <Card className='mt-4'>
                    <CardHeader >Assigned Batteries</CardHeader>
                    <div className='md:grid grid-cols-3 gap-4 p-4'>
                        {batteryData.map((battery, index) => (
                            <div key={index} className=' p-4 bg-secondary rounded-md'>
                                ...
                                <div className='flex gap-2'>
                                    <h1>Status: <span className={`${battery.status === "not issued" ? "text-secondary-foreground " : battery.status === "user" ? "text-green-400" : "text-amber-400"} font-bold`}>{battery.status}</span></h1>

                                </div>
                                <h1>Manufacturer: <strong>{battery.manufacturer}</strong></h1>
